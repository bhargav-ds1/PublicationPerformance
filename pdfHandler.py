...
class pdfHandler(Handler):
    ...
    def ref_from_pdf2txt_DE(self,filename):
        with fitz.open(filename) as doc:
            ...
            te = list(
                map(lambda a: a.split('3.4 Planung des Teilprojekts')[0] if '3.4 Planung des Teilprojekts' in a else a,
                    te))
            te = list(filter(lambda a: 'Begutachtete Publikationen' not in a, te))
            te = list(
                map(lambda a: a.split('Andere Veröffentlichungen')[0] if 'Andere Veröffentlichungen' in a else a, te))
            te = list(
                map(lambda a: re.split('\s\d{2,3}\s\w+\d+\s', a)[0] if re.search('\s\d{2,3}\s\w+\d+\s', a) else a, te))
            ...
            if len(te) > 5:
                with open(filename.split('.')[0] + '.txt', 'w', encoding='utf-8') as f:
                    for item in te:
                        f.write("%s\n" % item)
                    print(
                        "Completed extracting references from pdf(%s) to text file\nConverting the text file to .bib and "
                        "extracting the references from bib to CSV." % filename)
            else:
                print("The references could not be extracted from pdf file(%s)." % filename)

    def ref_from_pdf2txt_EN(self,filename):
        with fitz.open(filename) as doc:
            # print(doc.page_count)
            text = ''
            flag = 0
            ...
            te = list(
                map(lambda a: re.split('\s\d{2,3}\sProject', a)[0] if re.search('\d{2,3}\sProject', a) else a, te))
            te = list(map(lambda a: re.split('\s\[\d{2,3}]\s', a)[0] if re.search('\s\[\d{2,3}]\s', a) else a, te))
            # print(len(te))
            # print(te[0])
            if len(te) > 5:
        ...
        for filename in self.filenames:
            with fitz.open(filename) as doc:
                ...
                if detect(doc.get_toc()[0][1]) == 'en':
                    self.ref_from_pdf2txt_EN(filename)
                if detect(doc.get_toc()[0][1]) == 'de':
                    self.ref_from_pdf2txt_DE(filename)
...
if __name__ == '__main__':
    ...
    pdfH = pdfHandler('publikationstatistiken/pubStats/settings.yaml')
